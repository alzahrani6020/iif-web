...
- **`/admin-direct`:** مخصّص **للتطوير المحلي** (حقن جلسة بدون كلمة مرور عبر خادم `npm start`). على **Netlify** يُعاد توجيه `/admin-direct` إلى **`/fund-admin`** حتى لا يُعرَض سلوك التطوير على العام. **اختصارات مثل `/fund` و`/gov` و`/fund-admin`** تعمل محلياً بنفس **302** كما في `netlify.toml`.
- **`/admin`:** على **الخادم المحلي** يُفضَّل التحويل إلى `/admin-direct` (تفادي تكرار الدخول داخل iframe). على **Netlify** يفتح **`admin.html`** (إطار + أزرار فتح بنفس التبويب).

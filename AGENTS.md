...
| `OPEN-LOCAL-DASHBOARD.bat` | **ويندوز — فتح دخول اللوحة** (`/dashboard-entry`): نافذة بلا هيرو؛ iframe يحمّل `index.html` مع فتح اللوحة. محلي: `http://127.0.0.1:3333/dashboard-entry` — Netlify: `https://<موقعك>/dashboard-entry` |
| `financial-consulting/iif-fund-demo/dashboard-entry.html` | **صفحة دخول للوحة فقط** (شريط + iframe)؛ الإنتاج لا يُفعّل `local_dashboard` تلقائياً — سجّل الدخول داخل الإطار إن لزم |
...
## لوحة التحكم ومستودع منفصل

**الفصل «تماماً» عن هذا المشروع** يعني تطبيق إدارة لا يعيش داخل `financial-consulting/iif-fund-demo/index.html`: مستودع Git منفصل، نشر منفصل (نطاق فرعي أو خادم)، وربط بالبيانات عبر **API** أو نسخ بيانات محددة — وليس iframe أو نسخة أخرى من نفس الملف داخل الريبو. اللوحة الحالية **مدمجة** في الصفحة (آلاف الأسطر + سكربت مضمّن)؛ نقلها مشروعٌ إعادة بناء، وليس إعداداً سريعاً.

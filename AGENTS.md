...
| `OPEN-LOCAL-DASHBOARD.bat` | يفتح **`/admin-standalone`** — ملف **`admin-standalone.html`** بلا قسم الموقع العام (`#main-content`) |
| `financial-consulting/iif-fund-demo/admin-standalone.html` | **ملف HTML مستقل** يُولَّد من `index.html` بحذف الهيرو والأقسام العامة؛ نفس السكربتات واللوحة. اختصار: **`/admin-standalone`**. إعادة التوليد: `npm run build:admin-standalone` |
| `financial-consulting/iif-fund-demo/dashboard-entry.html` | شريط + iframe يحمّل **`admin-standalone.html`** (ليس `index.html`) |

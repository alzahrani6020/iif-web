...
| **المنصة `SIMPLE-GOVERNMENT-PLATFORM.html`** | بحث محلي؛ **بحث ويب** اختياري عند تفعيل المربّع أو `?web=1` في الرابط (على `localhost`/`127.0.0.1` + SearXNG + `npm start`). على الاستضافة العامة يبقى محلياً فقط ما لم يُضف بروكسي إنتاج. |

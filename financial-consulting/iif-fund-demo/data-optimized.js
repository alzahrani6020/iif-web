// Optimized Government Data System
// ============================================

class OptimizedGovernmentDataManager {
    constructor() {
    ...
    async initializeData() {
        // Optimized country data structure
        const countriesData = {
            'saudi-arabia': {
            ...
            'japan': {
                ...
                executiveAgencies: [
                    {
                        name: { ar: 'وكالة التعاون الدولي اليابانية', en: 'Japan International Cooperation Agency' },
                ...
                executiveAgencies: [
                    {
                        ...
                        director: { ar: 'برنار إيمي', en: 'Bernard Émié' },
                ...
                consulates: [
                    {
                        ...
                        consul: { ar: 'أوليفيير سانفيل', en: 'Olivier Sanvial' },
                        address: { ar: 'شارع الملك فهد, جدة', en: 'King Fahd Street, Jeddah' },
                        phone: '+966-12-667-0000',
    ...
    async searchCountry(query, language = 'ar') {
        ...
        for (const [key, country] of this.countries) {
            if (country.codes.some(code => code.toLowerCase() === queryLower)) {
                const result = this.formatCountryData(country, language);
    ...
    getAllCountries(language = 'ar') {
        const cacheKey = `all-countries-${language}`;
        ...
        const result = countries.sort((a, b) => a.name.localeCompare(b.name, language === 'ar' ? 'ar' : 'en'));
        this.cache.set(cacheKey, result);
        return result;
    }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = OptimizedGovernmentDataManager;
} else {
    window.OptimizedGovernmentDataManager = OptimizedGovernmentDataManager;
}

// Debug: Make sure it's available
console.log('⚡ Optimized GovernmentDataManager exported:', typeof window.OptimizedGovernmentDataManager);
